    ...
    <div className="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]">
      ...
      <div className="flex-grow bg-base-200 rounded-2xl p-6 overflow-y-auto custom-scrollbar">
        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
           Inventory <span className="badge badge-secondary">{inventory.length}</span>
        </h2>
        ...
        {inventory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center opacity-50">
            ...
            <p className="text-sm">Buy items from the market to see them here.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <figure className="px-2 pt-2">
                    <img src={template.image} alt={template.name} className="rounded-lg h-32 w-full object-cover" />
                  </figure>
                  <div className="card-body p-3 text-center">
                    <h3 className="text-sm font-bold truncate">{template.name}</h3>
                    <div className="badge badge-xs badge-outline mx-auto">{template.rarity}</div>
                  </div>
                </div>
              );
            })}
      ...
      {/* Item Details Aside */}
      <div className={`w-full lg:w-96 bg-base-100 rounded-2xl shadow-2xl p-6 flex flex-col transition-all duration-300 ${selectedItem ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-50 lg:opacity-100 lg:translate-x-0'}`}>
        {selectedItem ? (
          <>
            <button className="btn btn-circle btn-ghost btn-sm absolute right-4 top-4 lg:hidden" onClick={() => setSelectedItem(null)}>
                <X size={20}/>
            </button>
            ...
            <div className="flex-grow">
                ...
                <div className="flex gap-5 mb-6">
                    <figure className="bg-gradient-to-br from-neutral to-base-300 w-32 h-32 rounded-xl flex items-center justify-center flex-shrink-0 border border-base-content/10 shadow-inner">
                        <img src={selectedItem.template.image} alt={selectedItem.template.name} className="w-24 h-24 object-contain drop-shadow-xl hover:scale-110 transition duration-300" />
                    </figure>
                    <div className="flex flex-col justify-center">
                        <h2 className="text-xl font-bold leading-tight mb-2">{selectedItem.template.name}</h2>
                        <div className="flex flex-wrap gap-2">
                             <span className={`badge ${selectedItem.template.rarity === 'Ancient' ? 'badge-error' : 'badge-secondary'}`}>{selectedItem.template.rarity}</span>
                             <span className="badge badge-ghost text-xs">ID: {selectedItem.template.id}</span>
                        </div>
                    </div>
                </div>
            ...
            <div className="mt-6 pt-6 border-t border-base-content/10">
                <button className="btn btn-primary w-full btn-lg gap-2 shadow-lg shadow-primary/20" onClick={openSellModal}>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40 p-8">
            ...
            <p>Click on an item from your inventory to view details and sell options.</p>
          </div>
        )}
      </div>
      ...
      {isSellModalOpen && (
        <div className="modal modal-open bg-black/60 backdrop-blur-sm">
          <div className="modal-box relative">
            <button className="btn btn-sm btn-circle absolute right-2 top-2" onClick={() => setIsSellModalOpen(false)}>✕</button>
            <h3 className="text-xl font-bold mb-6">Sell {selectedItem?.template.name}</h3>
            ...
            <div className="space-y-6">
                ...
                <div className="form-control">
                    ...
                    <div className="relative">
                        ...
                        />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold opacity-50">NP</span>
                    </div>
                </div>
                ...
                <div className="form-control">
                    <label className="label">
                        <span className="label-text font-bold text-secondary">You receive</span>
                    ...
                    <div className="relative">
                        ...
                        />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold opacity-50 text-secondary">NP</span>
                    </div>
                </div>
                ...
                <div className="bg-base-200 p-3 rounded-lg text-xs space-y-1 opacity-70">
                    <div className="flex justify-between">
                        ...
                        <span>- {(parseFloat(buyerPays || '0') * 0.05).toFixed(2)} NP</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <button className="btn btn-primary w-full" onClick={handleConfirmSell}>
                    Confirm Listing
                </button>
            </div>
          </div>
