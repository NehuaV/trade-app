...
const { items: marketItems, listings: initialListings } = generateMockMarketData();

// User Balance (in Nexus Points)
export const balanceAtom = atomWithStorage<number>('nexus_balance', 1000.00);
...
export const marketListingsAtom = atomWithStorage<MarketListing[]>('nexus_listings_v2', initialListings);
...
export const myInventoryAtom = atom((get) => {
  ...
  return allItems.filter(item => item.ownerId === 'me' && !listedItemIds.has(item.id));
});
...
// Action: Buy Item
export const buyItemAtom = atom(
  null,
  (get, set, listingId: string) => {
    const listings = get(marketListingsAtom);
    ...
    // Transfer Ownership
    set(allItemsAtom, prev => prev.map(item => {
      if (item.id === listing.itemInstanceId) {
        return { ...item, ownerId: 'me', acquiredDate: new Date().toISOString() };
      }
      return item;
    }));
